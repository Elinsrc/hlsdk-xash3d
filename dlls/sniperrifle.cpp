...
void CSniperrifle::PrimaryAttack()
{
	...
	vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, Vector( flSpread, flSpread, flSpread ), 8192, BULLET_PLAYER_MP5, 1, 100, m_pPlayer->pev, m_pPlayer->random_seed );
	m_flNextPrimaryAttack = gpGlobals->time + 1.75;
	...
	m_flTimeWeaponIdle = gpGlobals->time + 68.0 / 38.0;
}
...
void CSniperrifle::Reload( void )
{
	...
	if (m_iClip == 0)
	{
		...
		m_flNextPrimaryAttack = gpGlobals->time + 2.25;
		m_flTimeWeaponIdle = gpGlobals->time + 2.25;
	}
	else
	{
...
void CSniperrifle::WeaponIdle( void )
{
	...
	if ( m_fInSpecialReload )
	{
		...
		m_flNextPrimaryAttack = gpGlobals->time + 49.0 / 27.0;
		m_flTimeWeaponIdle = gpGlobals->time + 49.0 / 27.0;
	}
	else
	{
		int iAnim;
		...
		else
		{
			...
			m_flTimeWeaponIdle = gpGlobals->time + 67.5 / 16;
		}
